...
class CertificateIssueBatchAPIViewTests(TestCase):
    ...
    def test_certificate_detail_endpoint(self):
        ...
        url = reverse('certificates:certificate-detail', kwargs={'pk': certificate_id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_certificate_issue_batch_endpoint(self):
        ...
        self.assertEqual(response.status_code, 400)  # Assuming it returns a 400 for missing data
